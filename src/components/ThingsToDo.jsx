...
export default function ThingsToDo({
  ...
  const displayedCount = isListOnly
    ? (propTodos || []).length
    : (itineraries || []).length;
  const todosList = isListOnly
    ? propTodos || []
    : (itineraries[selectedIndex] && itineraries[selectedIndex].todos) || [];
  ...
  useEffect(() => {
    ...
    if (user && user.email) {
      ...
      setItineraries(data);
      setSelectedIndex(
        Math.min(initialItineraryIndex || 0, Math.max(0, data.length - 1))
      );
    }
  ...
  function addPlace(place) {
    ...
    const todoItem = {
      ...
      description: newLoc.desc,
      preview: newLoc.preview,
    };
  ...
  function saveNote() {
    ...
    items[idx] = { ...items[idx] };
    items[idx].todos = items[idx].todos
      ? items[idx].todos.map((t) => (t === todo ? { ...t, notes: text } : t))
      : [];
    persist(items);
    setEditingNote(null);
  }
  ...
  async function searchCity(q) {
    ...
    try {
      ...
      const res = await fetch(url, {
        headers: { "User-Agent": "Aventra/1.0 (contact)" },
      });
    ...
    <div className="bg-white rounded shadow p-4">
            ...
            <div className="flex gap-2">
              <input
                ...
                onChange={(e) => {
                  ...
                  setTimeout(() => searchCity(e.target.value), 300);
                }}
              />
              <button
        ...
        <div className="mb-3 flex items-center gap-2">
          ...
          <button
            ...
            onClick={() => {
              ...
              const idx =
                selectedIndex >= 0 && selectedIndex < (itineraries || []).length
                  ? selectedIndex
                  : -1;
              if (idx === -1) return;
              const name =
                itineraries[idx] && itineraries[idx].title
                  ? itineraries[idx].title
                  : `Itinerary ${idx + 1}`;
              // eslint-disable-next-line no-restricted-globals
              if (
                !window.confirm(
                  `Delete itinerary "${name}"? This cannot be undone.`
                )
              )
                return;
              const items = [...(itineraries || [])];
      ...
      <div>
        {(!itineraries[selectedIndex] ||
          !(itineraries[selectedIndex].todos || []).length) && (
          <div className="text-sm text-gray-500">
            No saved things yet — add from search above.
          </div>
        )}
                ...
                <div className="flex-1">
                  <div className="flex items-start justify-between gap-3">
                    ...
                    <div className="flex-shrink-0 text-right space-y-1">
                      <div className="text-xs inline-block bg-gray-100 text-gray-700 px-2 py-0.5 rounded">
                        {(t.kinds || "").split(",")[0]}
                      </div>
                      <div className="text-xs text-gray-500">
                        {t.lat && t.lon
                          ? `${Number(t.lat).toFixed(2)}, ${Number(
                              t.lon
                            ).toFixed(2)}`
                          : ""}
                      </div>
                  ...
                  <div className="mt-2 flex gap-2 items-center">
                    ...
                    <a
                      ...
                      href={
                        t.xid
                          ? `https://opentripmap.com/en/poi/${t.xid}`
                          : `https://www.openstreetmap.org/#map=18/${t.lat}/${t.lon}`
                      }
                    >
                      Open
                    </a>
